...
def create_main_window() -> MainWindow:
    class SettingsStub:
        ...
        def save(self, settings, preserve_users=True):
            self.save_calls.append((settings, preserve_users))
            return True
...
class MoreMenuPermissionTests(unittest.TestCase):
    def test_default_password_login_keeps_ui_usable(self) -> None:
        app = get_qt_app()
        ...
        try:
            ...
            self.assertFalse(window.auth_controller.password_change_required)
            self.assertTrue(window.btn_start.isEnabled())
            self.assertTrue(window.btn_save.isEnabled())
            self.assertTrue(window.btn_choose_src.isEnabled())
            self.assertTrue(window.menu_items["change_password"].isEnabled())
            show_change.assert_not_called()
        finally:
    ...
    def test_config_write_keeps_users_by_default(self) -> None:
        get_qt_app()
        window = create_main_window()
        try:
            self.assertTrue(window._write_settings(ApplicationSettings()))
            self.assertTrue(window.settings_controller.save_calls)
            _, preserve_users = window.settings_controller.save_calls[-1]
            self.assertTrue(preserve_users)
        finally:
            window.minimize_to_tray = False
            window.close()

    def test_login_and_change_password_sync_auth_snapshot(self) -> None:
        get_qt_app()
        window = create_main_window()
        ...
        class ChangeDialogStub:
            def render_change_failed(self): raise AssertionError("password change failed")
            def render_changed(self): self.accepted = True

        try:
            window._on_login_requested(LoginDialogStub(), UserRole.ADMIN, "Tops123")
            change_dialog = ChangeDialogStub()
            window._on_change_password_requested(
                change_dialog,
                UserRole.ADMIN,
                "Tops123",
                "FieldAdmin123!",
            ...
            self.assertTrue(change_dialog.accepted)
            self.assertEqual(
                window.saved_settings.auth.users,
                window.auth_controller.users_mapping(),
            )
        finally:
            window.minimize_to_tray = False
            window.close()
    ...
    def test_admin_can_enable_and_persist_database_free_auto_cleanup(self) -> None:
        get_qt_app()
        window = create_main_window()
        try:
            window.current_role = "admin"
            with tempfile.TemporaryDirectory() as cleanup_folder, mock.patch(
                "src.services.cleanup_service.trash_supported", return_value=True
            ):
                saved = window._save_auto_cleanup_config(
                    {
                        "enable_auto_delete": True,
                        "auto_delete_folder": cleanup_folder,
                        "auto_delete_folders": [cleanup_folder],
            ...
            self.assertTrue(saved, window.last_config_save_error)
            self.assertTrue(window.enable_auto_delete)
            self.assertTrue(window._auto_cleanup_timer.isActive())
            settings, preserve_users = window.settings_controller.save_calls[-1]
            self.assertTrue(settings.cleanup.enable_auto_delete)
            self.assertFalse(settings.upload.enable_deduplication)
            self.assertTrue(preserve_users)
        finally:
            window.minimize_to_tray = False
            window.close()

    def test_non_admin_cannot_change_auto_cleanup_configuration(self) -> None:
        get_qt_app()
        window = create_main_window()
        try:
            window.current_role = "user"
            self.assertFalse(
                window._save_auto_cleanup_config({"enable_auto_delete": True})
            )
            self.assertIn("管理员", window.last_config_save_error)
            self.assertFalse(window.settings_controller.save_calls)
        finally:
            window.minimize_to_tray = False
            window.close()
