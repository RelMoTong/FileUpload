...
- 打包：PyInstaller 规格文件 `ImageUploadTool.spec` 读取 `src.__version__`，产物输出到 `dist/ImageUploadTool_v<version>/`。

运行与打包
----------
- 运行：`python -m src.main` 或直接使用打包后的 exe。
- 打包：`pyinstaller ImageUploadTool.spec --clean`，生成结果位于 `dist/`。
...
- `ImageUploadTool.spec`：包含资源/配置/版本，显式列出 Qt 与项目模块的 hiddenimports。
