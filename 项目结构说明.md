...
│  └─ make_icon.py
├─ ImageUploadTool.spec  # PyInstaller 打包配置
├─ qt_types.py              # Qt 类型补充
...
## 快速上手
- 开发运行：`python -m src.main`
- 打包发布：`pyinstaller ImageUploadTool.spec --clean`
...
- **ImageUploadTool.spec**  
  ...
  - 打包命令：`pyinstaller ImageUploadTool.spec --clean`，输出至 `dist/ImageUploadTool_v<version>/`。
