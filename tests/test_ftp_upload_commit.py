...
def test_confirmed_stor_and_matching_remote_size_succeeds(tmp_path: Path) -> None:
    ...
    assert len(ftp.rename_calls) == 1
    temporary_remote, committed_remote = ftp.rename_calls[0]
    assert temporary_remote.startswith("/image.bin.part-")
    assert committed_remote == "/image.bin"
    assert ftp.delete_calls == []


def test_rename_failure_removes_temporary_object_and_never_commits_final_name(
    tmp_path: Path,
) -> None:
    source = tmp_path / "image.bin"
    source.write_bytes(b"payload")

    class RenameFailureFTP(ConfirmingFTP):
        def rename(self, source: str, destination: str):
            self.rename_calls.append((source, destination))
            raise OSError("rename unavailable")

    ftp = RenameFailureFTP()
    result = _connected_client(ftp).upload_file_result(source, "/image.bin")

    assert not result.success
    assert result.status["code"] == "rename_failed"
    assert ftp.rename_calls[0][0].startswith("/image.bin.part-")
    assert ftp.rename_calls[0][1] == "/image.bin"
    assert ftp.delete_calls == [ftp.rename_calls[0][0]]


def test_remote_size_mismatch_removes_temporary_object(tmp_path: Path) -> None:
    source = tmp_path / "image.bin"
    source.write_bytes(b"payload")
    ftp = ConfirmingFTP(remote_size=2)

    result = _connected_client(ftp).upload_file_result(source, "/image.bin")

    assert not result.success
    assert result.status["code"] == "remote_size_mismatch"
    assert len(ftp.delete_calls) == 1
    assert ftp.delete_calls[0].startswith("/image.bin.part-")
