...
def _median_edge(values: list[int], *, tail: bool) -> float:
    if not values:
        return 0.0
...
def run_soak(
    ...
    log_growth = (logs[-1] - logs[0]) if len(logs) >= 2 else 0
    residual = []
    ...
    checks = {
        ...
        "sample_count_sufficient": len(samples) >= max(2, int(duration_seconds / sample_interval) - 2),
    ...
    return {
        ...
        "actual_duration_seconds": round(ended_at - started_at, 3),
        "sample_interval_seconds": sample_interval,
        "sample_count": len(samples),
        "exit_code": child.returncode,
        "timed_out": timed_out,
        "rss_first_edge_median": _median_edge(rss, tail=False),
        "rss_last_edge_median": _median_edge(rss, tail=True),
