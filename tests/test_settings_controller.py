...
def test_feature_gates_keep_dedup_frozen_but_allow_auto_cleanup_on_load(
    tmp_path: Path,
) -> None:
    ...
    assert DEDUPLICATION_FREEZE_ACTIVE is True
    assert settings.upload.enable_deduplication is False
    assert settings.cleanup.enable_auto_delete is True
    persisted = json.loads(config_path.read_text(encoding="utf-8"))
    assert persisted["enable_deduplication"] is False
    assert persisted["enable_auto_delete"] is True


def test_feature_gates_only_force_dedup_off_when_saving(tmp_path: Path) -> None:
    config_path = tmp_path / "config.json"
