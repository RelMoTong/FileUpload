...
def test_source_scan_yields_paths_incrementally_without_materializing_all(
    ...
    with mock.patch(
        ...
        worker, "_should_yield_source_path", return_value=True
    ) as identity_gate:
    ...
    assert identity_gate.call_count == 500
...
def test_network_jitter_pauses_once_and_resumes_after_recovery(tmp_path: Path) -> None:
    ...
    worker.status.connect(status_events.append)

    with mock.patch.object(
        worker,
        "_evaluate_smb_network_status",
        side_effect=["disconnected", "unstable", "good", "good"],
    ), mock.patch.object(
        worker._net_stop_event, "wait", side_effect=[False, False, False, True]
    ):
        worker._network_monitor_loop()

    assert status_events == ["paused", "running"]
    assert worker.current_network_status == "good"
    assert not worker.network_pause_by_auto
    assert not worker._paused
    assert "network" not in worker.pause_reasons
