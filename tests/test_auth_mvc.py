...
def test_default_password_login_is_persisted_and_keeps_business_actions_enabled() -> None:
    settings = FakeSettings()
    controller = AuthController(AuthService(), settings, model=AuthModel())
    ...
    assert permissions.btn_start
    assert permissions.btn_save
    assert permissions.btn_choose_src
